...
export function heraldicColors() {
  const tinctures = ['vert', 'azure', 'purpure', 'gules', 'sable']
  const metals = ['Or', 'argent']

  if (Math.random() > 0.5) {
    return [tinctures[Math.floor(Math.random() * 5)], metals[Math.floor(Math.random() * 2)]]
  } else {
    return [metals[Math.floor(Math.random() * 2)], tinctures[Math.floor(Math.random() * 5)]]
  }
}
