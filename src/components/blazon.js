...
export default function blazon(state) {
    ...
    if (design === 'divided') {
      ...
      switch (ordinaries) {
        ...
        case 'bend':
          ...
          if (chargeCount === 2) {
            d += `a bend ${colorB} between in chief ${describe(charge)} and in base ${describe(
              altCharge,
            )}`
          }
        ...
        case 'bend-sinister':
          ...
          if (chargeCount > 1) {
            d += `a bend sinister ${colorB} between in chief ${describe(charge)} and in base ${describe(
              altCharge,
            )}`
          }
          break
  ...
  const prep = string => {
    ...
    if (string.length > 280) {
      console.warn('String too long for twitter!')
    }
